...
void
ExceptionHandler(ExceptionType which , int badVaddr)
{
	...
	case SyscallException :
		...
		//START CODE CHANGES BY NACHLIFE
		case SC_Create: { //Create a Nachos file using FileSystem. Read in from register 4 to get the name 
				...
				fileSystem->Create(filename, 1024); // create the file
				break; }

		case SC_Open: { /* Open the Nachos file "name", and return an "OpenFileId" that can 
 				* be used to read and write to the file. Read in from register 4*/
				// Retrieve the address of the filename
				int fileAddress = arg1; // retrieve argument stored in register r4
				// Read file name into the kernel space
				char *filename = new char[100];
				...
				OpenFile *executable = fileSystem->Open(filename); // attempt to open the file
				if (executable == NULL) 
				{
				...
				break; }
		case SC_Close: { /* Close the file, we're done reading and writing to it. Read in from register 4*/
				...
				break; }
		case SC_Ls: { /*Ls() will simply give a directory
				listing that is equivalent to the ls –l in unix. That means, in this case, it should show file
				name, and file size for each file in the directory. Format it in a readable way. Read in from register 4*/

				// notes throughout directory.cc and directory.h for further implementation
				
				direct->Print();

				// I think were going to need to create a directory globally 
				// then add every file to th directory as they are initialized
				// and remove them each time the are finished being used

				// code to create the new directory:
				// make sure to #include directory.h

				Directory direct = new Directory(/* <shove a size here> */);

				// code to add file to directory:
				// needs to be used where files are initialized

				direct->Add(/* <"filename">*/);

				// code to remove file from directory when finished being used:

				direct->Remove(/* <"filename">*/);


				break; }
		case SC_Pwd: { /*Pwd() should return the current working directory, as it does in unix. However, since we are
				not using directory structure at this time, it should simply display the root path of ‘\’.  Read in from register 4*/
				


				break; }

		//END CODE CHANGES BY NACHLIFE
		case SC_Read :
			if (arg2 <= 0 || arg3 < 0){
				printf("\nRead 0 byte.\n");
			}
		...
		case SC_Exec :	// Executes a user process inside another user process.
	       ...
	       //Unprogrammed system calls end up here
			   printf("SYSTEM CALL: Unknown, called by thread %i.\n",currentThread->getID());
				printf("ERROR: Un-programmed system-call made. \n");
               break;
	...
	faultsCounter++;
	if (faultsCounter > 1035) {currentThread->Finish();}
	printf("ERROR: PageFaultException, called by thread %i.\n",currentThread->getID());
	printf("Number of faults: %d \n", faultsCounter);

	if(extraPrints) { printf("Page fault: process %d requests virtual page %d \n",currentThread->getID(),badVaddr); }
	...
	if(pagingType == 0){
		currentThread->space->DemandPaging(badVaddr);}
	else if (pagingType == 1){
		currentThread->space->PagingSchemes(badVaddr,pagingType);}
	else if (pagingType == 2){
		currentThread->space->PagingSchemes(badVaddr,pagingType);}
	//else if (pagingType == 3) {
		//currentThread->space->TwoLevelPaging(badVaddr);}
	else {currentThread->space->DemandPaging(badVaddr);}
	...
	case AddressErrorException :
		...
		printf("exception caused by page %d \n ", badVaddr/PageSize);
		if (currentThread->getName() == "main")
			ASSERT(FALSE);  //Not the way of handling an exception.
		if(currentThread->space)	// Delete the used memory from the process.
			delete currentThread->space;
